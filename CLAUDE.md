...
| **Database backend** | The tracker that holds the user's job-application records. The pipeline speaks in generic operations — query the queue, fetch a record, write a field, resolve the edit view — implemented by **the Notion adapter** (the only backend shipped today; its mechanics are the A1→A2→B read ladder and the `notion-*` MCP tools). Config names it via `database_backend` (default `notion`) + `database_id`/`database_edit_view_url`; the user-facing config is backend-neutral so a future adapter can reuse it. Notion is an implementation, not an assumption. Internal `$NOTION_*` shell vars are the adapter's names and stay as-is. |
