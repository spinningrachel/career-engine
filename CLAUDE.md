...
| CAREER_DATA pass-through pattern (`CAREER_DATA=${CAREER_DATA}`) | `skills/career-engine-new-application/SKILL.md`, `skills/career-engine-edit/SKILL.md` | `agents/humanizer.md`, `agents/cv-writer.md`, `agents/gatekeeper.md` | Subagents can't locate personal data at runtime |
...
| **Prioritization → full-intake always-overwrite.** The Prioritization pipeline writes provisional, JD-only values for `Role Summary`, `Location`, and `Priority` to every `New`-status role it processes (plus `JD Fetch Status` and `JD Body`, which stay write-only-to-empty/best-available). When a role is promoted to `Needs Research` and later reaches full intake, the career coach redoes these three from scratch using full research and **always overwrites** them — write-only-to-empty does not apply to these three at that point, joining `JD proof` and `Strategy` as the named always-overwrite exceptions. The coach never treats Prioritization's values as a draft to confirm or correct. | `skills/career-engine-intake/SKILL.md` Step 0.9a (the always-overwrite rule and the three per-property bullets for `Role summary`/`Location`/`Priority`) | `agents/role-prioritizer.md` (writes the provisional values), `skills/role-prioritizer/SKILL.md` (states the overwrite semantics as the reason the pipeline exists), `skills/database/SKILL.md` → Property Ownership (documents the provisional/overwrite relationship) | If write-only-to-empty were left in place for these three fields, every role reaching full intake would already have Prioritization's cheap values populated, so the coach's research-informed values would never get written — the pipeline would be permanently stuck with the cheap guesses. Verified this does not create a false coach-complete: Step 0.8's coach-complete checklist requires 13–14 fields (`Priority Reason`, `Role emphasis`, `JD proof`, `Keywords`, `Strategy`, `Role Type`, `Relationship type`, `Gap handling`, `Culture`, `Landscape`, `First Advertised`, etc.), none of which Prioritization writes — a Prioritization-only role is always missing at least 8–9 required fields and can never be mistaken for coach-complete. |
