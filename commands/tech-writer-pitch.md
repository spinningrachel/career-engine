...
2. **Spawn `cv-writer` with `option=reframe`.** Pass the full JD text and any instructions {{USER_FIRST_NAME}} has given (target person, channel preference, specific angle). `who-rachel-is.md` is loaded by cv-writer — do not pre-load it here.
