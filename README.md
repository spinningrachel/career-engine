...
Say "update my references" (or just share a material — an updated CV, a new testimonial, a portfolio piece, changed role facts) and the `update-refs` skill folds it in. This includes your positioning framework: people change, grow, and shift, and a direct statement in chat ("I'm now primarily pursuing a shift," "I started a new role") is enough to update `03-framework.md` — with the behavioral consequence named in the proposal, since the framework governs how every agent treats you. For bigger life shifts, it offers the relevant onboarding questions instead of patching piecemeal. It classifies each item against the reference map, proposes update / replace / add operations with a before/after preview, and writes nothing without your approval. It never assumes: ambiguous items and brand-new reference files trigger clarifying questions — a new file is only created once you've said what it's for, which tasks should load it, and when, and it gets wired into the agents' loading tables in the same session. Approved CV bullets and validated summaries are protected — conflicts are surfaced, never silently overwritten.
