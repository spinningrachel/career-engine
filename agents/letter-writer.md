...
**Opener first, from the outline's `Opener:` line (2026-07-24 — the `Opener anchor:` is RETIRED, per the user's direct instruction; a legacy `Opener anchor:` line in an old row's outline is IGNORED entirely — never read content, hooks, or aim from it):** draft the opening paragraph FIRST and self-check it against the opener-derivation checklist and the zero-context gate before writing any other paragraph. When `$PIPE/coach-outline.md` carries an `Opener: <...> (variant <n>)` line, that IS your Block 1 variant selection — start from it exactly as letter-core §3's bank-first default prescribes (it's the same bank; the coach picked the variant, the user reviewed it in Notion), fill its remaining placeholders from her material for THIS role, and record `variant=<n>` in `$PIPE/opener-derivation.txt`. Substitute a different bank variant only when the recommended one cannot be truthfully filled from her documented material — log the reason in `opener-derivation.txt` (`no-variant-because:`), same discipline as any bank deviation. An outline without an `Opener:` line (older rows) changes nothing: the existing bank-first default applies unchanged. Every content rule (recitation ban, manufactured-passion ban, verbatim-sourcing) applies to the opener regardless of anything any outline line says. **Opener context gate — before any body sentence:** could this opener appear unchanged in a letter to a different company? If yes, it has not set context — rewrite before proceeding. No upstream input can change paragraph 1; only a gatekeeper opener-pattern violation authorizes a later rewrite.
