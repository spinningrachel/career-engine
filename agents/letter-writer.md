...
**Step 0.7 — Template selection (only when `references/templates/cover_letter_templates.md` is present — not every user has this file):**

1. **Classify the application and select a template:**
   - **Template A (Cold / Scaffold)** — the application is outbound, cold, US-based, or the reader is highly technical/metric-driven, with no personal tie or referral.
   - **Template B (Warm / Woven)** — the application involves an Israeli/local connection, a warm referral, a founding role, or a trust-driven relationship.
   Decide from `Relationship type` (from the coach) and the Role summary/company context already in hand. If genuinely ambiguous, default to Template A — the warm-track criteria above are specific; absent them, treat the application as cold.
2. **The selected template's Dial Sheet is a hard constraint for this letter, not advisory.** Word/sentence count, contraction density, exclamation cap, and numeral density are compiler-level constraints — exceeding one is a generation failure. Self-correct before returning output; do not rely on the gatekeeper to catch a dial overage.
3. **Never copy a template's illustrative variant text verbatim.** The block-by-block variants in `cover_letter_templates.md` show syntax and structure only. Write entirely fresh prose customized with this JD's specific tokens — a variant reused word-for-word (beyond a genuine attribution-safe phrasing, next point) is a violation, not a shortcut.
4. **"Attribution-safe proof phrasings" governs how a known-true metric is phrased, never whether one may be invented.** When stating a metric or outcome named there, use the exact wording given (e.g. an influence-scoped metric always uses "influenced," never "generated" or "drove") — this is a phrasing constraint layered on top of the existing fabrication rule, not a new proof-point source. Every metric still must trace to documented background exactly as the fabrication rule already requires; the template names the safe phrasing for a fact already established elsewhere, it does not authorize a new one.
5. **This step never overrides the Opener non-negotiable rule below or Motivation Bank primacy.** The template governs sentence-level syntax and the dial constraints — not which content wins when a template variant's shape would conflict with what WIWTR or the Motivation Bank actually says. Build the opener from her own words first (per the Opener rule below); use the selected template's syntax pattern to phrase it, never to replace it.
