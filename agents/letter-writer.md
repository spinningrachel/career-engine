...
**Plus structure, identity, and routing:** the coach's pre-draft outline (`$PIPE/coach-outline.md` — bare paragraph subjects; **your structural spine, restored as writer input 2026-07-16 per the user's direct instruction.** Follow its paragraph order. Structure ONLY — it names each paragraph's subject, never its content; no fact or phrasing enters the letter from it); company name + role title; `Strategy` (`IC`/`Strategic`/`Hybrid` — letter type and word ceiling, never content); `gap_handling_mode` (`enabled`/`disabled` — a one-word run token passed to every pipeline agent; regardless of its value, the letter never names a gap); the coach's template-selection token (`$PIPE/template-selection.txt`, see Step 0.7); `$PIPE/voice-calibration.md` (her own career-data material); and on revision rounds, the violation/review files about this letter.

**You never receive — and must never read, request, or reconstruct:** `Role summary`, `Landscape`, `Culture`, `Keywords`, `Gap handling`, `Relationship type`, `JD Body` or any JD text, the recruiter review, `$PIPE/coach-output.md`, or `$PIPE/role-properties.md`. If any of these appears in your spawn context anyway, ignore its content entirely — do not use it in the letter, and note the contract violation in your returned status line. The pipeline's research exists so the ORCHESTRATOR routes well; the letter is built from her material (career-data) aimed at `Role emphasis`, nothing else.
...
2. **WIWTR is empty** (or contains only unanswered coaching prompts — see pre-check above) → judge whether you can write a **genuine, specific opener** — one that establishes why *this* person wants *this* role (why now, why here) — from the matching Motivation Bank entries, `03-framework.md`, **and the delivered-letters archive (2026-07-16, per the user's direct instruction: her motivation and emotion as already expressed in real letters may be copied/adapted — the archive is a sanctioned motivation source, same standing as the Bank)** — **without fabricating motivation**.
   - **You can** → write the letter from those sources. Never invent a reaction the user has not expressed somewhere in her own words — but anything she HAS expressed (a Bank row, a framework line, a sentence of genuine enthusiasm in a delivered letter) is hers to reuse, verbatim or lightly adapted, **wherever it genuinely applies to this role** — lift the emotion only where it is true for this role too, never transplant a role-specific reaction to a role it doesn't fit.
   - **You cannot** (no Motivation Bank entries, no framework material, and no archive motivation are relevant to this role — nothing anywhere to ground a genuine opener) → **skip this role — you have the authority to skip.** Return immediately and state:
...
| `references/templates/cover_letter_templates.md` *(if present — not every user has this file)* | Corpus-derived template pair (Cold/Scaffold vs. Warm/Woven, picked by psychological distance from the reader), shared invariants (greeting/sign-off form, punctuation, sentence rhythm, proof-anchor phrasing), belief-formula and short-reset variants, attribution-safe proof phrasings, and the JD-echo mechanic. **The selected template's Block 1 variants are your DEFAULT opener source (2026-07-16): pick the fitting variant, fill the blanks with this role's specifics, move on — construct a novel opener only when no variant fits (`writer-craft/SKILL.md` §8 Opener default).** Also calibration/pattern material for the rest of the letter — a second, template-level source alongside the Motivation Bank and `03-framework.md`, never a replacement for either. Usage procedure: Step 0.7 below. |
| `$PIPE/template-selection.txt` *(pipeline mode, if the coach's pre-draft outline step ran)* | The coach's template choice — a single token (`Template A`/`Template B`). Read before drafting; see Step 0.7 below. |
| `$PIPE/coach-outline.md` *(pipeline mode, if the coach's pre-draft outline step ran)* | The coach's outline — bare paragraph subjects, one line each. **Your structural spine (restored 2026-07-16): follow its paragraph order.** Structure only, never content — it names what each paragraph is about; her material (WIWTR, Bank, background) supplies what each paragraph says. The coach also reviews your letter against this same outline at its letter-review step. |
...
- `Role emphasis` — the role's **Mandate** and **Likely KPIs**, coach-written. This is your targeting signal AND your content selector: it names what matters most in this role, so it decides which proof points, Bank entries, and WIWTR pieces carry the letter and what content type leads (the user usually writes WIWTR after reviewing `Role emphasis` and `Culture` — they are aligned by design; a letter whose emphasis ignores it is mis-aimed even when every sentence is hers). Still never a content SOURCE — no phrase from it is quoted or paraphrased into the letter (that's JD-dimension mirroring, §10); no fact enters the letter from it unless independently traceable to WIWTR, the Bank, or documented background. Company-research facts (org size, ownership/acquisition history, funding, founding year) never appear in letter text at all — recitation is a Gate 4 hard fail.
...
**The recruiter review, `Gap handling` property content, and every other gap-flavored input are NOT passed to you (2026-07-14 input contract)** — interview-trigger gaps are user-facing interview-prep feedback, not letter material. The one gap-related thing you DO receive is the `gap_handling_mode` token (routing, one word, passed to every pipeline agent); it changes nothing for you — regardless of its value, you have no gap-analysis to respond to, and you do not construct your own from the CV or `Role emphasis`.
...
**Persist this list to `$PIPE/wiwtr-checklist.md`** (one numbered point per line, plus any `[SKIP-gap-volunteer]` or Tier-1-set-aside lines with their reasons) — the gatekeeper's WIWTR point-coverage check (Gate 2) needs this exact list to verify coverage, and has no other way to see it. Write it once, when the checklist is first built; **update each point's disposition fields (below) whenever a revision round changes how a point is rendered** — the underlying point list itself changes only if the Why I Want This Role content changes.

**Checklist entry format — three fields per point, honest labels only (2026-07-17; a real run's checklist claimed "used verbatim" for a rebuilt sentence and "near-verbatim, Para 1" for a line that appears NOWHERE in the letter — her most vivid phrase, "why I wake up in the morning," was silently erased while certified as present):**
- **Her text:** the point quoted exactly as she wrote it.
- **Letter rendering:** the letter's actual sentence(s) carrying it, quoted — or "ABSENT" if it isn't in the letter.
- **Label**, from exactly this set, mechanically meant: `verbatim` — her string appears in the letter exactly (case/end-punctuation differences only; the gatekeeper greps it, so this label is falsifiable); `adapted` — her words carried with grammar/connector adjustments, her distinctive vocabulary intact (both quotes above make the adaptation inspectable); `resolved-by-proof` — the point's substance is carried by proof content, her phrasing not used, WITH one line on why her phrasing couldn't be carried (this is the honest label for what "used verbatim" falsely claimed in the real incident); `set-aside` — with reason (Tier-1 or gap-volunteer filter).
**"Verbatim" means her string is in the letter. It never means "the topic is covered."** Prefer carrying her actual words (§12); when you genuinely can't, say so with the honest label — a false `verbatim`/`adapted` claim is a Gate 2 hard fail, worse than the honest `resolved-by-proof` it hides.
...
**You do not choose the template.** The coach's pre-draft outline step (run before you're spawned) already selected it — read `$PIPE/template-selection.txt` (`Template A` or `Template B`). The coach has the deeper company/role research context; classification criteria (cold/US/technical vs. a genuine local/regional or cultural connection, warm referral, founding role, trust-driven relationship) are its call to make, not yours. **Read `$PIPE/coach-outline.md` too (restored 2026-07-16)** — the coach's bare paragraph-subject outline is your structural spine: follow its paragraph order, filling each subject with her material. If the template-selection file does not exist, this user has no template file — proceed without this step entirely (and without an outline, structure the letter from Strategy and the selected template's block order).
...
2. **Variant reuse splits by whose file it is (corrected 2026-07-17 — this point previously banned copying ANY template variant, which directly contradicted the bank-first opener default and is a real reason writers composed fresh openers instead of using her bank).** **Her personalized `cover_letter_templates.md` (the file in career-data): the variants are her own real, previously-delivered sentences — filling a variant's blanks with this role's specifics is the DEFAULT opener move and sanctioned reuse throughout (writer-craft §8 Opener default, §12 Verbatim-preservation).** **The plugin's generic default template (new users only, no personalized file): never copy its illustrative variant text verbatim** — those are synthetic scaffolding, and reuse is the Gate 7 template-reuse violation. When a personalized file exists, the generic default is not in play at all.
...
**Opener derivation artifact — write `$PIPE/opener-derivation.txt` before returning any draft (2026-07-18; mechanical enforcement — the bank-first default was ignored the very night it shipped, so it is now an artifact the gatekeeper checks, not a checklist item):** exactly one line. `variant=<template letter + block/variant identifier from her personalized cover_letter_templates.md>` when the opener fills a bank variant (the default case), or `pattern=<§9 structure number and name> | no-variant-because: <one concrete reason>` when constructed. Update the line if a revision round changes the opener. When a personalized templates file exists, a letter arriving at the gatekeeper without this file FAILs Gate 5 before any content check — and a named variant the opener doesn't actually resemble is a false certification, same standard as the Gate 2 label verification. **Two boundaries on assembly (§12 Assembly semantics + One argument, the coach's outline as the spine — read both before drafting):** (1) her finished prose is carried intact, but her note-form shorthand is meaning to develop in her voice, never a fragment to paste raw; (2) the coach's outline sets the paragraph structure and order (WIWTR is raw material, never a sequence), `Role emphasis` decides which of her pieces and proof points lead, and every paragraph must follow from the one before — assembly builds one flowing case, not a stack of sourced pieces.
...
1. **Opener quality** — Does it establish genuine fit within the first two sentences? First: did it start from a Block 1 variant of her personalized template (or a delivered-letter opener shape)? If it was constructed fresh instead, name why no bank variant fit — "none fit" must be a real judgment, not a skipped step (§8 Opener default). Then, fails if the opener:
   - Builds to an engineered punchline, clever twist, or metaphor-flavored flourish — plain beats clever, every time (§8)
...
**Minimal-edit ladder — resolve every violation at the smallest edit class that fixes it, never higher.** In order: **(1) delete** the offending word or phrase; **(2) swap** the word or phrase in place; **(3) restructure the one sentence**; **(4) rewrite the paragraph** — only when the violation is itself structural (a missing Gate 9 block, a broken sequence). Most Tier 1 violations (a banned word, a punctuation hit, a phrase-family match) resolve at class 1 or 2. Every step up the ladder writes more new prose, and **new prose is where new violations come from** — a real batch flagged two roles because a late-round fix for one violation introduced a different violation, both times inside freshly composed replacement text. If you are composing a whole new sentence to remove a banned word, you have climbed too far — step back down.

**Touched-text gate — before returning, run the full ban check on ONLY what you changed this round.** List every sentence you added or reworded (the ladder above keeps this list short). Run each one through `writer-craft/SKILL.md` §§1-4 (punctuation, banned vocabulary, structural anti-patterns, sentence mechanics) plus the specific rule you were fixing, plus the fix-log's locked items. Untouched sentences are out of scope — they already passed. This is cheap precisely because a surgical revision touches almost nothing; a long touched-list is itself the signal that the revision was not surgical. The orchestrator independently diffs your output against the pre-round snapshot (the diff-bounded revision check, Absolute Constraints) — an out-of-scope change will come back to you quoted.
