...
4. **Using shift or step-down detection as a strategy-skip trigger** — Identifying that a role is outside the user's baseline function or below her documented seniority, then deferring, confirming, or returning empty or light framing. The shift or step-down is the strategic problem to solve, not a reason to stop. A role in the pipeline is a role the user has decided to pursue — the decision has been made; the job is to make the application work. When either detector fires: (a) note it in Patterns and Priority Reason; (b) actively mine `02-professional-background.md` and `03-framework.md` for transferable achievements, relevant skills, and stated passions that apply; (c) set `Strategy` (the letter-type Select) as normal, and lead the framing — `Role emphasis`'s `Capability match` lines and the Letter Outline — with the credibility-of-transfer argument, built from documented proof. `Role emphasis` and the Letter Outline are never empty, deferred, or lighter-than-normal for any role that reaches full research. Equally banned is the softer version of this failure: labeling the shift "friction," or ending a role's analysis with a "confirm you're comfortable applying as [X] before the pipeline runs" gate. The decision to pursue was made when the role entered the queue. A title the user has not held is a filter risk to handle in the materials, not a question to put back to her — see the skill's career-shift posture rule on no-hedging/no-friction.
...
| `${CLAUDE_PLUGIN_ROOT}/references/discipline-emphasis-signals.md` | Discipline → emphasis-signal map. Read before writing any `Role emphasis` (Option 1, 2) — anchors the what-does-this-JD-actually-weight read. |
...
2. **Basic fit signals** — assess from JD text and `pipeline-preferences.json`:
   ...
   - Location read — a plain JD text scan for stated location/timezone/authorization language (the compatibility-verdict property is retired, 2026-07-23; `my_location` still informs the Priority read)
...
5. **If Priority 5 or 6** (and no `--full-research` flag): mark this role a **triage exit** in your output and RETURN these for intake to write: `Priority`, `Priority Reason`, `JD Body` (if freshly fetched), `JD Fetch Status`, `Role Type`, and `Relationship type`. All of these are always-overwrite except `JD Body`, which is write-only-to-empty (see `career-engine-intake/SKILL.md` Step 0.9a — the source of truth for every property's write rule). Log in Patterns: "Triage exit [Priority X] — [Company] [Role Title]: [Reason]." **Do not proceed to Analysis for this role.**
...
### WIWTR question generation — RETIRED (2026-07-23)

Per the user's direct instruction ("the Coach Prompts that were supposed to be completely cancelled when we added in the letter outline"): generate NO coaching questions, return no `wiwtr_questions`, no `[COACH PROMPTS]` block, for any role. The Letter Plan below replaced this feature entirely.
...
Opener anchor: <terse third-person hook designation — the company-specific, non-transferable hook the first paragraph is built on; NEVER first-person or drafted prose>
...
**The `Opener:` line is a COPY/PASTE selection, never composition (2026-07-24, per the user's direct instruction: "the coach should have been copying/pasting either the verbatim opening changing only the role and/or hiring company OR copy/paste of a template with placeholder option").** Read the selected template's Block 1 Variants from `${CAREER_DATA}/references/templates/cover_letter_templates.md` (fallback: the plugin's `references/cover-letter-templates-default.md` only when the user's file is absent). Pick the variant whose shape best fits the anchor and paste it **verbatim**, substituting ONLY the `{ROLE}` and `{COMPANY}` placeholders with this role's actual values. Every other placeholder (`{SELF_LABEL}`, `{IDENTITY_IDIOM}`, `{SPECIFIC_RECENT_PROOF}`, `{REFERRER}`, ...) stays literal — those are HER material slots and only the letter-writer fills them, from her bank. Name the variant number in the trailing `(variant <n>)`. In a personalized templates file the variants ARE her own proven sentences — verbatim reuse is the entire point; in the generic default they are placeholder scaffolds the writer transforms per its own rules. You never compose an opening sentence, never blend variants, never fill a material slot.

**Block name (renamed 2026-07-23, per the user: "it doesn't need a title, but if it gets one it should just be Letter Outline").** The markers are `[LETTER OUTLINE` ... `[/LETTER OUTLINE]` — nothing else is added above or around the block. Consumers also still recognize the legacy `[COACH LETTER PLAN` marker on rows intaken before this rename; never write the legacy form again.

Rules: **Template** — you make this call, not the letter-writer; classification criteria are generic, never hardcoded to one user (same rule as Option 4a). **The `Template:` line is the bare token only** (e.g. `Template: Template B`) — no rationale, no dash-clause after it: the pipeline copies this line's value verbatim into `$PIPE/template-selection.txt`, which is specced as a single token; your reasoning, if worth keeping, goes in ONE Patterns line. **Opener anchor — a HOOK DESIGNATION, never letter prose (2026-07-23, from a real intake test the user caught):** a terse, third-person naming of the company-specific hook the writer builds the first paragraph on — e.g. `Opener anchor: the first-marketing-executive seat at the just-funded US-GTM inflection`. Non-transferable (fails if it could open a letter to a different company with only the name swapped), traceable to your research or the user's WIWTR/Motivation material. **NEVER:** first person of any kind ("I want...", "I'm..."), a drafted sentence in the candidate's voice, a motivation or desire claim on her behalf (motivation only ever comes from HER material — a coach-invented "I want to be the one who..." is a fabricated preference), or recitable research data (funding amounts, dates, headcounts — the writer is banned from reciting research facts in letter text, so an anchor built of them forces a violation downstream). **Confirmed real defect this closes:** a live intake returned "Reflectiz just raised $22M to carry client-side and web-exposure security into the US enterprise market, and I want to be the first marketing executive who turns the third-party-script blind spot into a story that market actually buys" — a fully-drafted, first-person, funding-reciting opening sentence, which the opener-anchor conformance gate would then FORCE into the letter. **Outline** — bare paragraph subjects only, one line per paragraph naming what it is *about*, never how to write it (same rule as Option 4a). Skip for triage-exit roles.
...
- **Analysis properties describe the role and company, never the candidate — one named exception (keystone, amended 2026-07-23).** `Landscape`, `Culture`, `Role summary`, and every research property are an objective intelligence brief about the role/company — never the candidate's name, "her letter," or letter strategy. `Role emphasis` = the fixed **Emphasis / De-emphasized / Capability match / Likely KPIs** structure, where the `Capability match` lines are the ONE sanctioned candidate reference (mapping the JD's weighted themes to her documented capabilities). Candidate-facing framing otherwise lives in exactly three places: `Gap handling`, the `Strategy` Select, and the Letter Outline. No interview prep, no positioning beyond the document stage.
...
**When this applies (fallback only since 2026-07-22):** Called by the new-application pipeline (before Step 5's letter-writer spawn) and edit pipeline (before Step E7's letter-writer spawn) ONLY when the role's `Why I Want This Role` contains neither a `[LETTER OUTLINE` block nor a legacy `[COACH LETTER PLAN` block — i.e. rows intaken before the Letter Plan moved to intake (Option 2). When the block exists, the pipelines parse it instead and this option is not called. Output contract addition: the first line of `$PIPE/coach-outline.md` must be `Opener anchor: <terse third-person hook designation — company-specific, non-transferable; same NEVER-rules as the Letter Plan's anchor: no first person, no drafted prose, no motivation claims, no recitable research data>`, the second line `Opener: <verbatim Block 1 variant of the selected template — same copy/paste rules as the Letter Plan's Opener line: only {ROLE}/{COMPANY} swapped, all other placeholders untouched> (variant <n>)`, followed by the paragraph-subject outline. No research, no Notion writeback. Read-only except for the two output files below.
...
- **Direct the writer toward content it cannot legitimately source (2026-07-16).** You receive `Role summary` and `Keywords`; the letter-writer, by its input contract, does not — its only role-analysis input is `Role emphasis`, and its content sources are the user's own material (WIWTR, Motivation Bank, career-data). Use `Role summary`/`Keywords` to judge whether the letter's *existing* material is aimed well — never to demand the letter name buyers, define the category, cite the role's metrics, or add any company/mandate fact the writer has no sanctioned source for. A review written that way forces the writer to invent positioning or recite research — the two failure families the input contract exists to prevent. **The strongest direction you can give — and usually the only one needed — points at HER material: which WIWTR point or Bank entry is underused, convoluted, out of her original sequence, or buried; quote her words and say where they should carry the letter.** If the letter genuinely cannot succeed without information the writer isn't allowed to have, say exactly that as a finding for the ORCHESTRATOR ("this role needs WIWTR input from the user / a richer Role emphasis"), not as a revision direction to the writer.
- **Introduce or police gaps.** Whether the letter acknowledges a gap is governed by the `Gap handling` property set at intake — not by you. **Never flag the letter for failing to acknowledge a gap, and never tell the writer to add gap acknowledgment.** When `Gap handling` is empty (gap handling is disabled for this run — there are no gaps), this is absolute: zero gap feedback of any kind.
