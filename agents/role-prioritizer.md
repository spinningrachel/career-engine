...
1. **Check the link is reachable and fetch the JD.** Reuse the exact Step 0.5 fetch ladder from `career-engine-intake/SKILL.md` — do not reinvent it, including its `content-exists` case: if there is no Job URL (or the URL fetch fails) but `JD Body` is already populated from a prior run, use that existing text directly — this counts as having JD text in hand for Steps 2-5 below (do not treat it as unfetchable, and do not write a fresh `JD Body` — Step 3's write-only-to-empty rule for `JD Body` already covers this). Only when every rung fails AND no existing `JD Body` exists: mark `JD Fetch Status = Unfetchable`, skip the remaining writes for this role (there is no JD text to score against), leave Status unchanged so it resurfaces, and move to the next role.
   ...
   **The user's own notes come first, and a fetched JD must be live (2026-08-12 — inherited from intake's Step 0.45 and Step 0.5 liveness check; both apply here in full).** Before fetching, read the row's user-authored free-text properties and page body. If any says the role is unadvertised, referral-only, or has no job description, **do not run the ladder** — score `Priority` from her stated facts, write a `Role Summary` from them, and never adopt a posting you find as this role's scope. If the ladder does run and returns a posting that contradicts her note, hers governs; note the conflict in the Step 4 summary rather than resolving it. And when JD text comes back from a rendering-capable or index-backed extractor, confirm the URL still resolves this run to that posting and not to a careers index or a 404 — cache proves a posting existed, never that it is live; a cached-only result is not "usable JD text in hand" for the gate above.
...
5. **Update `JD Fetch Status`** — the fetch outcome for this role: `Fetched`, `LinkedIn-blocked`, or `Unfetchable`. **If this role's JD text came from the Step 2.1 `content-exists` reuse path (an already-populated `JD Body`, not a fresh fetch this run), leave `JD Fetch Status` at its existing value instead of writing `Fetched`** — Prioritization has no way to tell a prior fetch from a user's manual paste (`Manual-entry`, coach-only — see `agents/career-coach.md`'s parity note), and overwriting either to `Fetched` would misrepresent it. Validate against the schema option list.
