...
| **Fabrication discipline.** | The fabrication rule is defined in `who-rachel-is.md` Section 1 — load it and apply it. Reviewer pressure does not license invention. Flags that cannot be resolved through reframing or surfacing documented experience are left unaddressed — not papered over. |
...
- No company names, client names, or conference names — descriptors only (banned list in `who-rachel-is.md` Section 1)
...
**`## TOOLS` section:** Include for Specialist and Builder roles only if the JD explicitly discusses tools or platform proficiency. Select relevant categories from `who-rachel-is.md` Section 8. Omit for Leader and Scaler roles regardless of JD content. Omit for any role type if the JD does not mention tools.
