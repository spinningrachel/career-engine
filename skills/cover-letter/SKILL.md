...
**The Motivation Bank is the letter's primary content and voice source; Why I Want This Role is the role-specific source — primary when present, but not mandatory.** The Motivation Bank (`02-professional-background.md` §5, the `| Tags | Motivation |` table of the user's verbatim words) is always loaded; the writer selects the role-relevant (tag-matched) entries and uses her own words first. Why I Want This Role (WIWTR) sits on top of the Bank as the role-specific motivation: when present, it is primary and everything she wrote there *must* appear somewhere in the letter; when empty, the pipeline writes from the role-matched Bank entries instead (and the letter-writer's Sufficiency Gate skips the role only when neither WIWTR nor any role-relevant Bank entry gives usable material — so by the time this skill runs, some usable material always exists). Whatever IS used must comply fully with every rule and prohibition in this skill. However, the order of the information provided must be according to all rules, prohibitions, and mandatory components and structures. Do NOT blindly copy/paste the user's input - not in the same order nor necessarily with the same exact wording. If something the user wrote (in WIWTR or a Bank entry) would produce a bad opener, violate the analyst paragraph ban, break a framing rule, or fail the context gate, set that piece aside — but place it elsewhere in the letter where it fits. The rules always win on placement and form; they never license dropping a WIWTR point.

**The integration model:** Coach output sets the frame — `Strategy` fixes the letter's structural type (`IC` / `Strategic` / `Hybrid`; see Letter Type below), and the coach context block (the Priority lines prepended to Why I Want This Role) plus Gap handling define what the letter must accomplish. The role-matched Motivation Bank entries (always) plus Why I Want This Role (when present) are the source of voice, angle, and raw material — used only where the content is genuinely usable and compliant. A letter that follows the frame but ignores the user's own words sounds like a template — but a letter that forces unusable content into a compliant shape is worse than either.
...
1. Open the user's own words for this role — **Why I Want This Role** when present, otherwise the **role-matched Motivation Bank entries** (`02-professional-background.md` §5). These are the source for the opener — not the JD, not coach output. The opening sentence must be explicit and give the hiring team immediate context for why the user is writing to them.
