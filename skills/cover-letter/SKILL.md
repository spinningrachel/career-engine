...
> **Mandatory load — shared voice rules.** Before writing any content, read `${CLAUDE_PLUGIN_ROOT}/references/shared-voice-rules.md`. This applies in all invocation modes — both when this skill is loaded by the `letter-writer` agent inside the pipeline and when it activates standalone via a direct `/career-engine:cover-letter` invocation. Shared prohibitions (AI syntax patterns, hollow phrases, sentence-structure bans) live there; this file holds only cover-letter-specific doctrine and surface deltas.
...
**WIWTR Content Types — classify before enumerating.** WIWTR can contain two kinds of content mixed together, and they must be handled differently:

- **Motivation content** — genuine first-person voice: reasons, feelings, reactions, experiences, or intentions about this role ("I love the brands I've worked with", "It would be an honor to serve my favorite city"). These are coverage items: they must appear in the letter. Use the user's own words.
- **Instruction directives** — sourcing instructions directed at the letter-writer, not voiced motivation: "Find in motivation bank and/or methodology information about multiple persona targeting", "Refer to professional background for info about regulations and compliance". Identified by imperative or third-person action verbs pointing at agent tasks ("Find", "Refer to", "Use", "Include", "See", "Check"). These are NOT coverage items — do not quote them in the letter; execute them instead.
- **Mixed items** — a single item containing both a directive clause AND genuine motivation content ("institutional or quasi-governmental context: refer to professional background for info about regulations and compliance; also you can discuss my flexibility and collaborative spirit - I'm like a chameleon when I interact with people"). Split: execute the directive portion; add only the genuine motivation portion to the coverage checklist.

This classification runs in Step 0.5 of the agent (before enumeration). Only motivation content enters the [WIWTR-N] coverage checklist. Directives are executed — they inform sourcing and expand the Bank material for this role, but they are never quoted as letter content.

**The Motivation Bank is the letter's primary content and voice source; Why I Want This Role is the role-specific source — primary when present, but not mandatory.** The Motivation Bank (`background/background-motivation-bank.md`, the `| Tags | Motivation |` table of the user's verbatim words) is always loaded; the writer selects the role-relevant (tag-matched) entries and uses her own words first. Why I Want This Role (WIWTR) sits on top of the Bank as the role-specific motivation: when present, it is primary and everything she wrote there *must* appear somewhere in the letter; when empty, the pipeline writes from the role-matched Bank entries instead (and the letter-writer's Sufficiency Gate skips the role only when neither WIWTR nor any role-relevant Bank entry gives usable material — so by the time this skill runs, some usable material always exists). Whatever IS used must comply fully with every rule and prohibition in this skill. However, the order of the information provided must be according to all rules, prohibitions, and mandatory components and structures. Do NOT blindly copy/paste the user's input - not in the same order nor necessarily with the same exact wording. If something the user wrote (in WIWTR or a Bank entry) would produce a bad opener, violate the analyst paragraph ban, break a framing rule, or fail the context gate, set that piece aside — but place it elsewhere in the letter where it fits. The rules always win on placement and form; they never license dropping a WIWTR point.

**The integration model:** Coach output sets the frame — `Strategy` fixes the letter's structural type (`IC` / `Strategic` / `Hybrid`; see Letter Type below), and the coach context block (the Screen lines prepended to Why I Want This Role) plus Gap handling define what the letter must accomplish. The role-matched Motivation Bank entries (always) plus Why I Want This Role (when present) are the source of voice, angle, and raw material — used only where the content is genuinely usable and compliant. A letter that follows the frame but ignores the user's own words sounds like a template — but a letter that forces unusable content into a compliant shape is worse than either.

**The integration rule:** Read `Strategy` first to fix the letter type, then read the coach context block to understand what this letter needs to accomplish. Then read the role-matched Motivation Bank entries and Why I Want This Role (when present) to find the user's voice, her specific angles, and her raw content. Write the letter so her own words are doing the strategic work the coach identified.

**Opener source — non-negotiable.**
The opener's content and angle come from the user's own words — Why I Want This Role when present, otherwise the role-matched Motivation Bank entries. Either way it is HER words, never invented. The user's genuine reaction to the role, her specific angle on this company, and the raw material she wrote goes first, in her voice — polished to be appropriate for formal writing, but not replaced with generic professional language. The Use-Case Structures tell you how to frame it — not what to say, not in what order, not in which paragraph. Take the user's input and place each part where it best serves the Use-Case Structure you choose.
...
**Beyond the opener — use the user's words throughout.** The opener draws from her own words (WIWTR when present, otherwise the role-matched Bank entries), but that material is NOT confined to the opener — and the opener is not its only job. Leverage it across the entire letter: wherever a piece of her content fits the structure — a proof paragraph, a transition, the close — work it in.

**MANDATORY (when WIWTR is present): every distinct point the user wrote in Why I Want This Role must appear somewhere in the letter.** Not thematically covered — actually present. The integration mechanism:
...
**When WIWTR is empty:** the selected role-matched Motivation Bank entries are the source. There is no [WIWTR-N] coverage requirement and no obligation to cover the entire Bank — use the role-relevant entries (in her verbatim words), placed where each does real work for the letter.
...
**Proof-point partitioning — run before drafting.** The CV and the letter draw from the same documented background, and the CV is written first — so list what the CV already spends (summary claims, bullet outcomes, metrics) before selecting the letter's proof. The letter's named proof comes from what the CV does NOT carry: the detail layers in the relevant `background-role-facts-*.md` file ("What you built / delivered" runs deeper than any bullet), adjacent engagements, stories and decision logic behind the bullets, `background-testimonials.md` testimonials, `background-portfolio.md` portfolio artifacts. If every documented proof point is CV-spent, enhance the strongest one (story behind it) instead of importing a restatement — and a named non-numeric specific (deliverable, methodology, artifact) satisfies any need for concreteness when all numbers are CV-spent.

**The user's own words govern proof selection.** Which outcomes to lead with, what the proof paragraphs need to demonstrate — this comes from Why I Want This Role (when present) and the role-matched Motivation Bank entries. If the coach prepended a context block (above the `---` separator), treat it as directional framing; treat everything below the separator as the user's voice and content source. If the story is interesting but misses a coaching cue, include it where it fits — setting a piece of her content aside entirely is a last resort, reserved for content that is non-compliant or genuinely unusable.
...
- **Opener:** Same rules as Type 1 — personal reaction, sourced from the user's own words (WIWTR when present, otherwise the role-matched Motivation Bank entries). If provided in WIWTR, the user's reaction may center on *why she believes in the organization* or her long-held conviction about the domain, in addition to capability match.
...
FIRST AND FOREMOST, AND COMPLETELY MANDATORY AND NON-NEGOTIABLE: The first sentence must name the role the user is applying for. The role does not have to lead the sentence, but it must be clear and specific. The opener MUST express the user's genuine reaction to this specific role, sourced from her own words — Why I Want This Role when present, otherwise the role-matched Motivation Bank entries — in her actual tone, vocabulary, and phrasing, polished for formal writing. If WIWTR is empty, draw the opener from the role-matched Bank entries; if WIWTR is present but too sparse for a clean opener, check the Bank for relevant material first, and only if nothing usable is found write `[{{USER_FIRST_NAME}} TO FILL IN]` (the case where neither WIWTR nor the Bank has usable material is handled upstream — the letter-writer's Sufficiency Gate skips the role — so this path can assume some usable material exists). Do not construct an opener from your own reading of the JD. Do not write strategy analysis, expert claims, or methodology descriptions as openers.
...
1. Open the user's own words for this role — **Why I Want This Role** when present, otherwise the **role-matched Motivation Bank entries** (`background/background-motivation-bank.md`). These are the source for the opener — not the JD, not coach output. The opening sentence must be explicit and give the hiring team immediate context for why the user is writing to them.
2. Identify the substance: what angle did the user take? What specific reactions, comparisons, or ideas did she provide (in WIWTR or the Bank entries)? What is she actually saying she wants from this kind of role?
...
4. **The user's notes are raw material — not a draft paragraph.** WIWTR and Bank entries are notes: first-person, often unpolished, written to herself. Your job is to write a paragraph from them, not to copy them. Extract the angle, the specific reaction, the thing she's saying — then write a letter paragraph that expresses that. The paragraph should sound like a letter, not like her notes.
...
Step 4 says "write a paragraph from it." That means: from what she actually wrote — the words, phrases, reactions, and comparisons that appear in Why I Want This Role OR in the role-matched Motivation Bank entries. It does not mean: use her notes as a prompt and construct a paragraph using your own reasoning about what she would say or feel.

**Sparse notes are not a gap to fill.** Do not expand sparse notes into a full paragraph by inventing reactions she did not write. If Why I Want This Role is sparse — one sentence, a few words, no specific angle or reaction — draw from the role-matched Motivation Bank entries instead; her standing words there are equally valid opener source. If neither WIWTR nor the Bank has material for this role, the letter-writer's Sufficiency Gate already skipped the role upstream — so by the time you reach this step, some usable material exists. The soft fallback `[{{USER_FIRST_NAME}} TO FILL IN]` remains only for a present-but-thin opener after the Bank has been checked and found to add nothing.

**The traceability test — run before finalising the opener:** Can you point to a specific word, phrase, or reaction — in Why I Want This Role OR in a role-matched Motivation Bank entry — that every sentence of the opener is built from? If you cannot trace a sentence directly back to something she actually wrote, that sentence is invented. Delete it, or (if no usable material remains) replace the opener with `[{{USER_FIRST_NAME}} TO FILL IN]`.
...
**Failure mode C — expansion of sparse notes:** Why I Want This Role contains one sentence or a few words. The opener is a full paragraph expressing enthusiasm, specific reactions, and personal observations she did not write. This is fabrication. The model treated sparse notes as permission to invent what she would have said. It is not "helping" — it is putting words in her mouth and calling them her voice. The correct move when WIWTR is this sparse is to draw from the role-matched Motivation Bank entries (her standing words for roles like this); only if the Bank adds nothing usable does the soft fallback `[{{USER_FIRST_NAME}} TO FILL IN]` apply.

The fix for A and B: read her notes (WIWTR and the role-matched Bank entries), understand what she's saying AND how she's saying it, then write a paragraph that says it well in her words. The fix for C: pull from the Bank; placeholder only if the Bank adds nothing.
...
2. **Opener paragraph.** Choose from the Use-Case Structures below. The user's reaction or observation comes first. Her background enters in service of that, not as the subject. Source her reaction from Why I Want This Role when present, otherwise from the role-matched Motivation Bank entries. If no documented reaction exists in either, write `[{{USER_FIRST_NAME}} TO FILL IN]` — do not manufacture enthusiasm.
