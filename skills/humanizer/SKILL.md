...
3. **Load the pattern list.** Load `${CLAUDE_PLUGIN_ROOT}/skills/writer-craft/SKILL.md` — read the `[ALL]` sections (punctuation, vocabulary, structural bans, sentence mechanics). Load this skill's own Mechanics section below for the step-by-step procedure and the Quantitative Final Gate (sentence range, paragraph variance, passive density, hedging count) that must be verified before returning output. *(Prefix all plugin file paths with `${CLAUDE_PLUGIN_ROOT}/` — bare relative paths fail when this agent runs as a subagent.)* The delivered letters (or the durable calibration file) also serve the instinct check in Step 5 below.
...
**Optional: real computed targets instead of the static defaults above.** `skills/humanizer/scripts/corpus-stats.py` is a generic, standard-library-only script that computes sentence-length distribution, subject-first-opener %, contraction rate, digit density, Flesch-Kincaid grade, and type-token ratio from any directory of letter files. When a delivered-letters archive exists (`${CAREER_DATA}/references/delivered-letters/`), run it against that directory — `python3 ${CLAUDE_PLUGIN_ROOT}/skills/humanizer/scripts/corpus-stats.py <archive_dir>` — and use its output as the real, per-user targets for burstiness and sentence-length checks 1-2 above, in place of the fixed defaults. The static defaults above remain the fallback when no archive exists or the script cannot run.

---

## Voice Calibration Protocol

**Pipeline mode (`$PIPE/voice-calibration.md` provided):** Read it directly. Calibration is complete — no archive read needed, no direct career-data calibration read needed. This is a copy of the durable `${CAREER_DATA}/references/voice-calibration-coverletters.md` file, made by the orchestrator before this spawn (new-application Step 4.9 / edit Step E6.8) when that durable file existed at run start.
...
1. **Durable file present** at `${CAREER_DATA}/references/voice-calibration-coverletters.md`: read it directly. Calibration is complete — no archive read needed.
