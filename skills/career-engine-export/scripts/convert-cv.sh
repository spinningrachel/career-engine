...
if [ -z "${CV_FOOTER}" ] || [ ! -f "${CV_FOOTER}" ]; then
  echo "ERROR: CV footer not found at '${CV_FOOTER:-<empty>}'. Pass the resolved \$CV_FOOTER (career-data references/static-cv-footer.md) as argument 4." >&2
  exit 1
fi
