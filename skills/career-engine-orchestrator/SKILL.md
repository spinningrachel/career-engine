...
2. **Config key completeness:** verify all 13 required keys are present and non-empty in `pipeline-preferences.json`: `notion_database_id`, `output_folder`, `cv_template`, `draft_dir_url_base`, `output_dir_prefix`, `default_language`, `word_templates_path`, `notion_needs_editing_view_url`, `gap_handling`, `location_compatibility`, `favorite_brands`, `preferred_job_sites`, `local_job_sites`. For each missing or empty key, list it by name and stop: "career-data config is incomplete — run `/career-engine:setup --phase 5` to fill in: [list of missing keys]."
