...
CV and cover letter text is produced only by cv-writer, letter-writer, and the humanizer. The orchestrator's editing authority is limited to the mechanical inline fixes explicitly authorized in the gatekeeper loop steps (swap a word, remove one phrase, reorder paragraphs — zero creative judgment). It never composes sentences, merges drafts, picks between bases, or assembles a final document from parts. **If a writer keeps failing after every cap in the pipeline steps is exhausted: halt that role before export or Notion writeback — never deliver it.** No DOCX is produced, no Notion status advances.

**The halt must be written to disk at the moment it happens — never held only in the orchestrator's own in-context memory for later reporting.** The instant a role halts (at any cap clause in either pipeline's steps), append an entry to `$OUTPUT_DIR/halted-roles.json` — same read-parse-push-write append discipline as `state.json` (Step 7b in the main pipeline, Step E10's sibling in edit): read the existing file if present (start with `{"roles": []}` if it doesn't exist yet), push a new object, write the full array back. Never `cat >` over an existing file. Entry shape:
```json
{"company": "<name>", "role_title": "<title>", "track": "<cv|cover_letter>", "step": "<the step where the halt triggered, e.g. '5.2'>", "reason": "<one-line unresolved-violation summary>", "revision_log": "<path to the per-role revision-log file>"}
```
This is the ONLY mechanism that makes the halt reportable later — a run can process many more roles, and possibly many more turns, between this halt and Steps 8/9c at the very end. Relying on conversational memory across that gap is exactly the fragile pattern this plugin's own `state.json` crash-recovery design and R-41 file-based-handoff convention already exist to avoid elsewhere; it must not be reintroduced here. `orchestrator-post-run.md`'s Step 9c (`roles_halted` in run-metrics) and Final Chat Delivery (the mandatory halted-role addendum) both read from this same file — it is their only data source, not "the run state" in the abstract.

This applies to that role only — every other role in the run's queue proceeds normally. **This replaces an earlier "deliver the last passing version flagged for manual review" policy, which shipped a real, confirmed-non-compliant cover letter and CV in production (2026-07-10) under the belief that logging a violation in a revision-log file counted as flagging it — nothing downstream ever surfaced that log distinctly from a normal completion, so it read, in Notion and in chat, exactly like a clean pass.** A hand-assembled document has bypassed every gate and must never be exported, same as before.
...
**Template resolution — fixed filenames, no config key (2026-07-04 fix).** `cv_template` and `word_templates_path` are no longer config keys. Set `$CV_TEMPLATE` = `${CAREER_DATA}/references/templates/cv.dotx`, `$CV_TEMPLATE_BRIEF` = `${CAREER_DATA}/references/templates/cv-brief.dotx` (Brief CV Type — resolved once here like the others, though only actually required for a role whose per-role Step 0.type resolves to `Brief`), `$CL_TEMPLATE` = `${CAREER_DATA}/references/templates/cover-letter-template.dotx`, `$CV_TEMPLATE_HE` = `${CAREER_DATA}/references/templates/cvHe.dotm`, `$CL_TEMPLATE_HE` = `${CAREER_DATA}/references/templates/he-letter.dotx`, `$CV_FOOTER` = `${CAREER_DATA}/references/static-cv-footer.md`, `$CV_FOOTER_HE` = `${CAREER_DATA}/references/static-cv-footer-he.md` — always these fixed relative paths, never an external OS path, never a config lookup. `$CV_TEMPLATE`/`$CL_TEMPLATE`/`$CV_FOOTER` are required for any export — if any of the three doesn't exist, stop with: "career-data is missing `references/<filename>` — run `/career-engine:setup --phase 5` to restore the default templates." **`$CV_FOOTER` (2026-07-09 fix)** replaces the plugin's own `skills/career-engine-export/static-cv-footer.md`, which `convert-cv.sh` previously hardcoded for every CV export — confirmed in the shipped repo to have accumulated one real user's actual Education/Languages content, meaning every installation was silently appending someone else's real degree/university info onto every exported CV. The plugin's own copy is the new-user blank-placeholder fallback only, copied to career-data at setup (`career-engine-setup/SKILL.md` Phase 5), same as the `.dotx` templates. `$CV_TEMPLATE_BRIEF`/`$CV_TEMPLATE_HE`/`$CL_TEMPLATE_HE`/`$CV_FOOTER_HE` are optional at this preflight stage — if `$CV_TEMPLATE_BRIEF` is missing, Brief CV Type export is unavailable (collect into `CONFIG_HEALTH`; `career-engine-new-application`'s own Step 0.type additionally fails fast per-role if a role actually resolves to `Brief` and the file is still missing at that point). If `$CV_TEMPLATE_HE`/`$CL_TEMPLATE_HE`/`$CV_FOOTER_HE` is missing, Hebrew export for that document type is unavailable (collect into `CONFIG_HEALTH`).
