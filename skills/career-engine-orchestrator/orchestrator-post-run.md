...
A flagged role SHIPPED its md and DOCX (flag-and-deliver policy) but got no Notion status change; this line — `[Company] — [Role Title]: ⚠ delivered with unresolved violations — [specific violation(s)] — review before sending; see <revision-log>` — plus the feedback file's warning section are where its state is reported, so it cannot be silently missed the way a same-status-as-clean-pass role could be before this fix. If a role was flagged on the CV but its cover letter (or vice versa) passed cleanly, say so explicitly rather than reporting the whole role as flagged.
