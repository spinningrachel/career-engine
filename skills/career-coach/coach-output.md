...
#### <Company> — <Role Title>
- **Role emphasis:** (fixed four-line structure — `coach-analysis.md` is the authority; no confidence tags, no strategy, no CV type, no company facts, no rank commentary, ever)
  - **Emphasis:** <the JD's most-weighted themes, ranked, each with its JD evidence named>
  - **De-emphasized:** <what the JD conspicuously underweights or omits vs. the role-type norm>
  - **Capability match:** <which of the user's documented capabilities the weighted themes map to, through the company's real GTM/tech — name the mechanism>
