...
**Opener-anchor conformance — RETIRED (2026-07-24, per the user's direct instruction: "Get rid of the hook. the opener anchor. that's what's causing this mess.").** The 2026-07-22 conformance check (opener must be built on the coach's `Opener anchor:` hook) is removed with the anchor itself — the anchor slot was the confirmed vector by which coach-composed research prose entered the letter path through a hard gate. Do not check the opener against any coach-outline anchor line, including a legacy one still present in an old row's outline — a legacy `Opener anchor:` line is ignored entirely, never enforced. Opener quality remains fully covered by: Gate 5's opener patterns and non-transferability forcing function (Tier 1), the opener source check below, and the writer's own zero-context test. **Opener source check (2026-07-24; plain-language rename of the same-day "opener-variant conformance" — the user's rule it enforces: the opening sentence is written from her outline/templates/verbatim banks, with fresh composition only after career-data is exhausted):** when `$PIPE/coach-outline.md` carries an `Opener: <...> (variant <n>)` line, confirm `$PIPE/opener-derivation.txt` records either that variant (`variant=<n>`) or a different bank source with a logged `no-variant-because:` reason. FAIL only when the derivation record contradicts the outline with no logged reason, or is missing entirely — never fail the opener's content for using a different piece of her material when the reason is logged; her material and the content bans always outrank the recommendation. This check replaced the per-role coach strategic letter review (removed 2026-07-22 per the user's instruction).
