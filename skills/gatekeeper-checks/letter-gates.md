...
**Opener-anchor conformance (2026-07-22 — hard fail; runs whenever a letter plan was passed):** when the spawn passed `$PIPE/coach-outline.md` and its first line is `Opener anchor: <...>`, the opening paragraph must be BUILT ON that anchor — the anchor's specific hook (the named company fact, event, product, or user connection) must be recognizably present in the first paragraph. FAIL if the opener is built on a different hook, or on no hook. **Built ON, never copied FROM (2026-07-23):** conformance means the anchor's underlying hook is recognizably present — it never requires (or excuses) reproducing the anchor's own phrasing, figures, or claims; an opener that recites the anchor's research data (e.g. a funding amount) or its first-person motivation wording still FAILs the recitation and manufactured-passion checks — a prose-injected anchor never overrides a content ban. The anchor comes from the user-reviewed Letter Outline (intake Write C — `[LETTER OUTLINE]` markers since 2026-07-23, legacy `[COACH LETTER PLAN]` on older rows) or the Option 4a fallback; user edits to the plan in Notion are authoritative — never fail an opener for following the user's edited anchor's HOOK (her edits govern aim, not exemption from content bans). This check replaced the per-role coach strategic letter review (removed 2026-07-22 per the user's instruction).
