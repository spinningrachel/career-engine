...
These use {{USER_FIRST_NAME}}'s documented experience. Every claim in the strong version traces to `who-rachel-is.md`.
...
- **Every metric must trace to `who-rachel-is.md`.** If a number is not documented, do not write it. Write the named outcome without the number.
...
These apply to CV bullets and summaries. For every generic claim removed, replace it with what {{USER_FIRST_NAME}} actually did — a specific role, outcome, or named result from `who-rachel-is.md`. Never invent replacement metrics.
