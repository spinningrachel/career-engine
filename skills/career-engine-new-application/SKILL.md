...
2. **Final gatekeeper pass** — spawn `gatekeeper` with `option=cover-letter` on this exact text, passing `CAREER_DATA=${CAREER_DATA}`, `gap_handling_mode=$GAP_HANDLING_MODE`, the cover letter path `$PIPE/letter-final.md` to read, `Role summary`, `Strategy` (same as Step 5.2), the user's Why I Want This Role content (same as Step 5.2), the final CV path `$PIPE/cv-final.md` to read, `$PIPE/wiwtr-checklist.md` to read if it exists (same as Step 5.2), `Template selected=<same value passed at Step 5.2, or omit if that file was absent>`, and `OUTPUT_PATH=$PIPE/gatekeeper-cl-<round>.md`.
