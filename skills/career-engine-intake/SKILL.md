...
**Every downstream read in this run** — Step 0.6 priorities, the Step 0.8 coach-complete check, the Step 0.9a write-only-to-empty rule — reads from `$PIPE/hold-role-properties.md`, never from an in-memory fetch result or a rendered table. If every rung of the read ladder fails, stop and report — never treat it as zero results, and never improvise `notion-search` to enumerate the queue (R-39). The adapter also carries the all-paths rules (no view creation, no Bash/Grep on A2/B results, target-status filtering).
...
**Rule: write only to empty properties** (the always-writes — `JD proof`, `Role summary`, `Location`, `Priority`, and the `Why I Want This Role` coach context block — are the named exceptions below). **The "current Notion value" check is the pre-write snapshot already captured in `$PIPE/hold-role-properties.md` (Step 0b) — do not issue a fresh `notion-fetch` per property to check emptiness; that snapshot is the authoritative pre-write state, the same source Write B already uses for the WIWTR write-only-to-empty check below.** If already populated in that snapshot — skip it. Do not overwrite any existing value. `N/A` counts as populated.

**`Role summary`, `Location`, and `Priority` are always-overwrite, not write-only-to-empty — same class as `JD proof`.** These three are exactly the fields the Prioritization pipeline writes provisionally to a `New`-status role before it reaches full intake (see `skills/database/SKILL.md` → Property Ownership and the cross-file-contract row in `CLAUDE.md`). By the time a role reaches this step, any existing value in these three fields is Prioritization's cheap, JD-only estimate — never a value to confirm, correct, or preserve. The coach redoes all three from scratch using full research (competitive landscape, deep JD decoding, corroborated location, favorite-brand boost) and its value always supersedes whatever is there, exactly like the `JD proof` anti-fabrication overwrite below. Do not skip these three because they're already populated — check whether the coach produced a value, not whether Notion already has one.
...
- `Role emphasis`, `Keywords`, `Strategy`, `Relationship type`, `Person who Advertised Role (if not Hiring Manager)` — write if empty. (Triage-exit roles skip these — only full-research roles write them.)
