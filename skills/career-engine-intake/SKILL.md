...
5. If every candidate above was genuinely tried and none found it: stop and report — "career-data skill is required but was not found. Install it via Customize → Skills in the Desktop app."
...
Report the count to the user: "Found N roles in Needs Research status." **If the count is 0: stop and report that — suggest running Prioritization first** ("run prioritization" promotes `New`-status roles to `Needs Research`, which is what intake reads) **rather than improvising a substitute for either pipeline.** This is not a hypothetical: a real production run found nothing ready and, instead of stopping here, hand-rolled Prioritization's job itself (direct Notion writes, no `role-prioritizer` spawn) and then hand-rolled intake's own Steps 0.4–0.9e too — skipping the `$PIPE` convention, the Step 0.8.5 gatekeeper Coach Output Check, and the Step 0.9e outreach-map write entirely. **Neither pipeline's mandatory gates are optional just because a prerequisite step is inconvenient or a subagent spawn failed — stop and report the specific blocker instead of substituting an ad hoc path around it, the same discipline already named for `career-data` direct writes and coach-output hand-edits elsewhere in this file and in `CLAUDE.md`.** If the query call returns a tool error or an unparseable response rather than a result array, stop and report the error to the user — do not treat it as zero results. Do not wait for a response — proceed immediately to the next step.
...
- **When `gap_handling_mode = enabled` (default):** all nineteen fields must be populated — `Role summary`, `Priority`, `Priority Reason`, `Role emphasis`, `JD proof`, `Keywords`, `Strategy`, `Role Type`, `Relationship type`, `Gap handling`, `Culture`, `Landscape`, `Company Stage`, `Hiring manager's role`, `Manager role confirmed`, `No incumbents in this function`, the **`Why I Want This Role` coach context block**, **`Location`** (only if the DB has a `Location` property — skip this field from the check entirely if it doesn't), and **`First Advertised`**. **`JD Body` is required only when this run's Step 0.5 marked the role with any `url-fetched*` marker** (a live fetch succeeded this run) — skip this field entirely for a `content-exists` role (already has JD Body from a prior run) or a `needs-manual` role (no fetchable content at all).
...
**Rule: always overwrite.** The coach was invoked on this role because the user wanted it (re-)analyzed — the assumption is a deliberate refresh, for any reason. Any existing value already in Notion, whether hand-entered, left by a prior coach run, or written provisionally by Prioritization, is treated as stale by default and superseded by whatever the coach produced this run. This is the pipeline's default for every coach-owned property; the three named exceptions below are the only fields that still hold their prior value. **The "current Notion value" check (for the three exceptions) is the pre-write snapshot already captured in `$PIPE/hold-role-properties.md` (Step 0b) — do not issue a fresh `notion-fetch` per property to check emptiness; that snapshot is the authoritative pre-write state.** `N/A` is a value like any other for an always-overwrite property — write it if that's what the coach returned, even over an existing non-`N/A` value.

**Exceptions — these three remain write-only-to-empty** (check the pre-write snapshot; if already populated there, including `N/A`, skip the write):
...
- `Priority` — **Select field** (not a number field; values are the strings `"1"` through `"6"`). Validate the coach's returned value against the live schema option list before writing. **Always overwrite** — even if Prioritization or a prior run already set a value, the coach's fresh score (full research, favorite-brand boost, Open Application hard floor) supersedes it. If the role was coach-skipped (already coach-complete per Step 0.8), do not write at all — leave unchanged. In a mixed batch, apply per role individually.
