...
- **When `gap_handling_mode = enabled` (default):** all fourteen fields must be populated — `Role summary`, `Priority`, `Priority Reason`, `Role emphasis`, `JD proof`, `Keywords`, `Strategy`, `Role Type`, `Relationship type`, `Gap handling`, `Culture`, `Landscape`, **`Location`** (only if the DB has a `Location` property — skip this field from the check entirely if it doesn't), and **`First Advertised`**.
- **When `gap_handling_mode = disabled`:** thirteen fields — same list minus `Gap handling`, which is NOT required and must NOT block coach-complete status.
...
**Rule: write only to empty properties** (the always-writes — `JD proof`, `Role summary`, `Location`, `Priority`, and the `Why I Want This Role` coach context block — are the named exceptions below). For every other property, check the current Notion value first. If already populated — skip it. Do not overwrite any existing value. `N/A` counts as populated.
