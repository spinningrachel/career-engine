...
| **Antecedent integrity after every cut — required.** When you delete or rewrite a sentence, the cut can orphan a pronoun or demonstrative later in the letter — "that adoption," "this shift," "it," "they," "those" whose referent you just removed. After each cut, re-read the *following* sentences and confirm every pronoun and demonstrative still points at something the letter still names. This is a re-verification step, not a new ban — it runs on every edit you make in any step. | A pronoun or demonstrative whose antecedent was removed by an earlier cut, leaving it pointing at nothing. | Cutting the sentence that introduced "the adoption problem," then leaving "and that adoption is exactly what I'd own" two sentences later. | Restore the referent in the surviving text, or rewrite the dangling pronoun to name the thing directly. |
