...
- The coach's **outline** (`$PIPE/coach-outline.md`) from Step E6.9 — the writer's structural spine (restored 2026-07-16, per the user's direct instruction); bare paragraph subjects, structure only. The coach still reviews the letter against the same outline at Step E7.4.
...
- **Mandatory, one line per role flagged this run per the Absolute Constraints' flag-and-deliver policy** (E3.5, E5.5, E7.3, the E7.4 coach-directed cap, E7.7, or E8.5) — never omitted when at least one role was flagged: `[Company] — [Role Title]: ⚠ delivered with unresolved violations — [specific violation(s) in one clause] — review before sending; see revision-log-<file>.` A flagged role ships its md and DOCX AND advances its Notion status like any other (2026-07-17 amendment); this addendum plus the halted-roles.json entry and the feedback file's warning section are where its flagged state is reported.
