...
**`CV Type` recommendation moved out of `Role emphasis` (2026-07-23, per the user's direct instruction).** When `pipeline-preferences.json` → `cv_type.mode` is `"Variant"`, the coach returns a standalone `CV Type` value (Detailed or Brief, with a one-line rationale — see `skills/career-coach/coach-analysis.md`'s CV Type Recommendation Matrix and its two tiebreaks), and intake writes it to the per-role `CV Type` select **write-only-to-empty** — the user's own hand-set value is never overwritten. `Role emphasis` never carries a CV-type clause anymore; when `cv_type.mode` is `Detailed` or `Brief`, no `CV Type` value is returned or written at all.
