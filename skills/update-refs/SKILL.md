...
| Word templates (.dotx/.dotm) — CV and cover letter | `references/templates/`, fixed filenames only: `cv.dotx`, `cover-letter-template.dotx`, `he-letter.dotx` (Hebrew CL), `cvHe.dotm` (Hebrew CV, note `.dotm` not `.dotx`) | Personalized version only — personal data, never the plugin repo (see line 83 / R-37). **No config key** (`cv_template`/`word_templates_path` are retired, R-38, 2026-07-04) — a replacement template is installed by renaming/copying it to the fixed filename above, nothing else to update. |
| Cover letter templates, formulas, invariants (openers, belief-formula variants, proof phrasings) | `references/templates/cover_letter_templates.md` | Calibration/pattern material — never overrides the Motivation Bank or the fabrication rule; letter-writer treats it as secondary to the user's own verbatim words |
| Personal brand source material (who she is professionally, voice profile, value proposition) | `references/voice-and-identity/personal-brand-context.md` | Read by `skills/personal-brand/SKILL.md` before Capability A |
| LinkedIn content strategy (topic authority areas, post guidelines) | `references/voice-and-identity/linkedin-post-strategy.md` | Read by `agents/linkedin-post-writer.md` and `skills/content-orchestrator/SKILL.md` |
