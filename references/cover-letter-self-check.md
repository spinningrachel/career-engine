...
Run every item in order before returning output.

**Note on syntax:** Sentence-level syntax rules (em dashes, tricolons, -ing appendages, passive voice, clause structure, etc.) are enforced by the humanizer agent, which runs after this check. Do not fail the letter here for syntax violations. Flag them as advisory in the revision log if you notice them, but do not loop the letter-writer for syntax. The humanizer handles it.
...
- [ ] Is the candidate's Q&A content being used to serve the strategic proof Role emphasis identified — or has it replaced it?
...
### Opening
- [ ] **Source check — do this first:** Does the opener express the angle and content the candidate provided in her page body or Q&A? The substance — what she said she wants to communicate, her key phrases, her specific angle on this role — must be present. The opener must not express a different framing derived from the agent's own reading of the JD. Check specifically: (a) are her key phrases and ideas present? (b) does any sentence express something she did not write and did not ask for? If yes to (b), remove it.
...
- [ ] **No triple-used proof points:** Any specific metric or named outcome that appears in both the CV summary AND an experience bullet must not also appear in this letter.

**Fabrication traps:**
- [ ] No claims about scope, attribution, or seniority not documented in `01-candidate-rules.md` or `02-candidate-background.md`
...
- [ ] No addressing JD dimensions or requirements by name: "I can also speak to the [X dimension] directly" — write as natural experience instead
- [ ] No manufactured opener hook — if the reaction is genuine, state it directly; if not, write `[CANDIDATE TO FILL IN]`
- [ ] No expert-claim or strategy-analysis opener constructed by the agent — only content the candidate wrote
