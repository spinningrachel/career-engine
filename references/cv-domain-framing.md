...
Role facts, documented metrics, fabrication traps, and approved phrasings for each company are in `candidate-rules.md` Sections 1 and 7. The rules below are writing mechanics applied at draft time — they are not repeated in those files.
...
- **Builder, Leader, or any cyber/devtools role:** break out the relevant early roles as full entries — see `candidate-rules.md` Section 7 for which roles to break out.
...
**Page limit:** Two pages maximum. Bullet limits per role are defined in `candidate-rules.md` Section 7 — do not exceed the documented maximum for any role.
